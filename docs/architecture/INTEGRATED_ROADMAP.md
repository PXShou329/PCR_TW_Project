...
11. B4＋A6-0／A6：先修 Princess Arena Gate correctness，再建立 Planner 與成熟三隊案例。
12. 正式 A4＋B5 content expansion：Timeline 由 2/6 補足、建立 News data／UI；不得把
    RP-B5-1 的薄切片誤標為整個 milestone 完成。
13. B6 Shadow → B7 Review／Audit／Rollback。
14. A7：Data Gate A／B／C PASS；其中 A3 expansion 尚須把 PVE 由 2 個成熟關卡擴至 5。
15. B8：writable SSOT 原子切換。
16. B9／B10：效能、安全、備援與 Production Gate G。
...
- RP-A2 當時的 PostgreSQL typed closure 為 8 條 timeline、14 個 atomic steps；目前 RP-B5-1
...
- 正式 A3 PVE expansion 已交付 RP-A3／RP-A4 兩個成熟關卡；PVE Gate B 的數量條件
  ...
  slice 實際服務化，但 Timeline Gate 仍為 2/6、News 尚缺，Arena Gate 仍為 0。下一步依
  Gate-bearing content 需求推進 B3＋A5 expansion，並在 B4＋A6 內容前先完成 A6-0 Gate
  correctness。Data Gate
...
- 回滾點：A2 research core 使用 `rp-a2-2`；A2＋B2 通過 Compose CI 的完整切片使用
  ...
  `rp-b3-d0-1`；RP-B5-1 候選完成私人 CI 後才建立新的 immutable `rp-b5-1` tag。
  A5→A4→A5 依 [`A5_ROLLBACK_RUNBOOK.md`](../operations/A5_ROLLBACK_RUNBOOK.md)，
  B5→A5→B5 依 [`B5_ROLLBACK_RUNBOOK.md`](../operations/B5_ROLLBACK_RUNBOOK.md)。任何已有
