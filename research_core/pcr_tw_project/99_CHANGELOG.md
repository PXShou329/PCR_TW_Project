...
| 日期 | 檔案 | 變更摘要 | 來源 |
|---|---|---|---|
| 2026-08-10 | B4-0 structural candidate：37、38、README、99 | 定義 P-Arena v5 typed serving 為 mature-only；47 header-only／實際 mature `N=0` 是合法 fail-closed 狀態，非缺口自動補隊。實開 2021 巴哈 `LOSE／WIN／WIN` 圖組後只可作 D single-report 候選，仍缺完整 case WIN、三筆 mature 39、18／必要強化與獨立來源；2024 模板／部分防守、2026 問答、未精確定位 YouTube 均不足，JP／Bilibili 依範圍排除。全部標 `DEFERRED_NOT_CANONICAL`，不進 39／47／92／93；Gate A／B／C、blocking warnings 與 CSV 統計不變。 | Builder 實開候選複核＋mature-only contract |
| 2026-08-10 | A6-0 Maintainer P1/P2：00、01、37、38、README、99、tools/* | 補上 92 `evidence_confidence` 全域 A–E enum；P-Arena case WIN direct Evidence 只接受 A–D，E 不成熟。VERIFIED `case_win_claim_id` 不得跨 case 重用，Gate 計數排除所有重用列且 D warning 依 Claim 去重；47 的 fresh TW source URL hostname 必須覆蓋 case Evidence hostname，已知 published date 不得晚於 Evidence verified date。Mutation 新增 M118–M124，並把合法正控改為 synthetic matching 46 source；aggregate 額外合法 Claim／Evidence 仍由 exact-set closure 拒絕，Gate 門檻未降低，47 維持 0 真實案例。 | Maintainer 重現＋Builder 最小 fail-closed 修正 |
| 2026-08-10 | A6-0 Princess Arena Gate correctness：00、01、11、14、31、35–38、47、README、99、tools/* | 47 由 20→24 欄，新增三個 team result Claim 與一個完整 P-Arena 整體 WIN Claim；`VERIFIED` 必須精確閉合三筆同 TW／同 environment／無序隊伍 pair 的 mature 39 列，以及一筆 ACTIVE TW `parena` `SOURCE_FACT` direct Claim。敵我各 15 人不得重複、全員須為 18 AVAILABLE，designated Claim／direct Evidence 集合、fresh 46 source、日期上下界及防守三隊重排去重皆 fail-closed；D 級整體 WIN 可計 Gate B，但僅產生一筆 blocking warning 且不得通過 Gate C。39 的 `required_upgrade_check` 全列 enum 納入 `NOT_APPLICABLE`，成熟仍只接受 PASS；新增 M109–M117（含合法正控），Gate 門檻未降低，47 維持 header-only／0 真實案例。 | Builder＋Maintainer fail-closed 稽核 |
| 2026-08-10 | B5-1 Gacha limited provenance：40–42、44、99、README、tools/* | 41 由 35→36 欄，在 `limited` 後新增 `limited_claim_id`；三筆已確認限定身分分別指定既有 JP 官方 A Claim，兩筆直播未明示者維持 `UNKNOWN` 且指定欄空白。新增 ST85c，要求 known limited 的 row→Claim→Evidence 為 ACTIVE／gacha／JP／OFFICIAL／A direct closure；UNKNOWN 禁止夾帶 Claim。ST85a 同步強化 `OFFICIAL_OVERRIDE` 為 ACTIVE TW 官方 direct closure，不再只信 Claim scalar。MATURE 的相對優先級改為證據缺口敘述，移除持有角色條件與偏強推薦語氣；新增 M107／M108 攔截無 provenance 的限定斷言與弱 Evidence override；Gate 門檻未降低。 | Builder fail-closed 最小修正 |
| 2026-08-09 | Freshness-0：02、12、15、40–42、44、45、91–93、99、README、tools/* | 台／日官方索引與指定正文實開核對至 2026-08-09；ev045／046／047／049／#3943 補齊，新增 ev116–ev124。官方直播只以精確投影片 timestamp 登錄ヴァンピィ（サマー）與ティア卡池及路線圖；兩角限定身分未明示，41 新列固定 `limited=UNKNOWN / maturity=RESEARCH / NOT_EVALUATED`，不給抽取建議。RESEARCH 的 `relative_priority` 固定 NOT_EVALUATED、future_upgrade 僅 UNKNOWN／NOT_EVALUATED，M106 防止自由文字偷渡推薦。CURRENT-0802 由 CURRENT-0809 取代；45 的 001／002 設 CHECKED，003／004 設 STALE，Community 誠實達 2/2；個人寶石門檻與社群自譯名不匯入。Baseline/release metadata=2026-08-09；next_review_due=2026-08-16。 | Builder 實開台／日官方影片與社群正文 |
